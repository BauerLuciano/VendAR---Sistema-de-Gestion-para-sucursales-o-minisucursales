...
// Agregamos las nuevas rutas de Caja Diaria y Cajas Físicas
const links = [
    { nombre: 'Dashboard', ruta: 'dashboard', icono: 'dashboard' },
    { nombre: 'Caja Diaria', ruta: 'cajadiaria.index', icono: 'caja_diaria' }, // <-- NUEVO: Gestión de turnos y arqueos
    { nombre: 'Punto de Venta', ruta: 'pos.index', icono: 'pos' },
    { nombre: 'Historial de Ventas', ruta: 'ventas.index', icono: 'ventas' }, 
    { nombre: 'Ingresos de Stock', ruta: 'ingresos.index', icono: 'ingresos' },
    ...
    { nombre: 'Proveedores', ruta: 'proveedores.index', icono: 'proveedores' },
    { nombre: 'Sucursales', ruta: 'sucursales.index', icono: 'sucursales' },
    { nombre: 'Cajas Físicas', ruta: 'cajas.index', icono: 'cajas_fisicas' }, // <-- NUEVO: CRUD de Cajas del sistema
...
<template>
    <div class="w-64 bg-slate-900 h-screen fixed left-0 top-0 shadow-2xl flex flex-col z-50">
        ...
        <nav class="flex-1 px-4 py-6 space-y-2 overflow-y-auto font-sans custom-scrollbar">
            <Link v-for="item in links" :key="item.nombre" 
                :href="item.ruta !== '#' ? route(item.ruta) : '#'"
                :class="[
                    item.ruta !== '#' && route().current(item.ruta.split('.')[0] + '*')
                    ? 'bg-sky-600 text-white shadow-lg shadow-sky-600/20'
                    : 'text-slate-400 hover:bg-slate-800 hover:text-white'
                ]"
                class="flex items-center gap-3 px-4 py-3 rounded-xl font-bold transition-all duration-200 group text-sm"
            >
                <span class="w-5 h-5 flex items-center justify-center">
                    ...
                    <svg v-if="item.icono === 'cajas_fisicas'" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M20.25 7.5l-.625 10.632a2.25 2.25 0 01-2.247 2.118H6.622a2.25 2.25 0 01-2.247-2.118L3.75 7.5M10 11.25h4M3.375 7.5h17.25c.621 0 1.125-.504 1.125-1.125v-1.5c0-.621-.504-1.125-1.125-1.125H3.375c-.621 0-1.125.504-1.125 1.125v1.5c0 .621.504 1.125 1.125 1.125z" /></svg>
