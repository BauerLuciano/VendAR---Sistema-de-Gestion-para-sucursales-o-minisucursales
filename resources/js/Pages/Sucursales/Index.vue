...
const toggleEstado = (s) => {
    ...
    Swal.fire({
        ...
        text: `La sucursal "${s.nombre}" cambiará su estado de actividad.`,
        icon: 'warning',
                        ...
                        <table class="w-full text-left border-separate border-spacing-y-2">
                            ...
                            <tbody>
                                <tr v-for="s in sucursales" :key="s.id" 
                                    ...
                                    :class="{'opacity-50 grayscale': !s.estado}">
                                    
                                    <td class="p-4 font-mono font-bold text-sky-800">#{{ s.id }}</td>
                                    <td class="p-4 uppercase font-bold text-slate-700">{{ s.nombre }}</td>
                                    <td class="p-4 text-xs font-black text-sky-600 uppercase">{{ s.tipo.replace('_', ' ') }}</td>
                                    
                                    <td class="p-4 text-center">
                                        <span :class="s.estado ? 'text-emerald-600 bg-emerald-50' : 'text-rose-600 bg-rose-50'" class="px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest shadow-sm border border-current opacity-70">
                                            {{ s.estado ? 'Activa' : 'Baja' }}
                                        </span>
                                    </td>
                                    
                                    <td class="p-4">
                                        <div class="flex justify-center gap-3">
                                            <button @click="abrirDetalle(s)" title="Ver Detalle" class="bg-sky-100 text-sky-600 p-2 rounded-xl hover:bg-sky-600 hover:text-white transition-all shadow-sm">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                                            </button>

                                            <button @click="abrirEditar(s)" title="Editar" class="bg-amber-100 text-amber-600 p-2 rounded-xl hover:bg-amber-500 hover:text-white transition-all shadow-sm">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                                            </button>

                                            <button @click="toggleEstado(s)" title="Cambiar Estado"
                                                :class="s.estado ? 'bg-rose-100 text-rose-600 hover:bg-rose-600' : 'bg-emerald-100 text-emerald-600 hover:bg-emerald-600'" 
                                                class="p-2 rounded-xl hover:text-white transition-all shadow-sm">
                                                <svg v-if="s.estado" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636" /></svg>
